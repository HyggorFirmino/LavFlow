...
import { DRYER_LIST_IDS, INITIAL_LIST_ORDER, INITIAL_LIST_TITLES, TAG_COLORS, WASHER_LIST_IDS } from './constants';
import { fetchClients } from './services/apiService';
import { BoardData, Card, CardHistoryEvent, Client, LaundryProfile, List, TagDefinition, ToastNotification, User } from './types';
...
const ToastMessage: React.FC<ToastMessageProps> = ({ notification, onDismiss }) => {
  ...
  const BORDER_COLORS: Record<ToastNotification['type'], string> = {
...
const App: React.FC = () => {
  ...
  useEffect(() => {
    ...
    fetchClients()
    .then(apiClients => {
        // Mapeia os dados da API para o formato esperado pelo ClientsPage
        const mappedClients = apiClients.map((c: any) => ({
          customer: c.name,         // Use c.name or another field as appropriate
          fullName: c.name,         // ou c.fullName se já vier assim
          documentId: c.document,   // ou c.documentId se já vier assim
          cellphone: c.phone,       // ou c.cellphone se já vier assim
        }));
