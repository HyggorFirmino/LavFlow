...
const StatCard: React.FC<{ icon: React.ReactNode; title: string; value: string; detail?: string }> = ({ icon, title, value, detail }) => (
  <div className="bg-white/80 dark:bg-slate-800/80 backdrop-blur-md rounded-xl shadow-lg border border-laundry-blue-200 dark:border-slate-700 p-4 md:p-6 flex items-center space-x-3 md:space-x-4">
    <div className="bg-laundry-teal-100 dark:bg-laundry-teal-500/20 p-2 md:p-3 rounded-full flex-shrink-0">{icon}</div>
    <div className="min-w-0">
      <p className="text-sm md:text-base text-laundry-blue-800 dark:text-slate-200 font-semibold truncate">{title}</p>
      <p className="text-xl md:text-3xl font-bold text-laundry-blue-900 dark:text-slate-100 truncate">{value}</p>
      {detail && <p className="text-xs md:text-sm text-gray-500 dark:text-slate-400 truncate">{detail}</p>}
    </div>
  </div>
);
  ...
  return (
    <div className="flex-grow p-4 md:p-6 lg:p-8 overflow-y-auto">
      <div className="max-w-7xl mx-auto space-y-8">
        <h1 className="text-xl md:text-3xl font-bold text-laundry-blue-900 dark:text-slate-100">Dashboard de Análise</h1>
