...
const ListView: React.FC<ListViewProps> = ({ cards, boardData, tagsMap, onEditCard, onDeleteCard, currentUser }) => {
      ...
      <div className="max-w-7xl mx-auto">
        <div className="flex justify-between items-start mb-6">
          <div className="flex items-center">
            <ListBulletIcon className="w-8 h-8 text-laundry-teal-500 mr-3" />
            <div>
              <h1 className="text-3xl font-bold text-laundry-blue-900 dark:text-slate-100">Visão em Lista</h1>
              <p className="text-laundry-blue-700 dark:text-slate-300">Exibindo {filteredCards.length} de {cards.length} pedidos</p>
            </div>
          </div>
        </div>
            ...
            <table className="min-w-full divide-y divide-laundry-blue-200 dark:divide-slate-700">
              <thead className="bg-laundry-blue-100/70 dark:bg-slate-800/70">
                <tr>
                  ...
                  <th scope="col" className="px-6 py-3 text-left text-xs font-bold text-laundry-blue-800 dark:text-slate-300 uppercase tracking-wider">Entrada</th>
              ...
              <tbody className="bg-white dark:bg-slate-800 divide-y divide-laundry-blue-100 dark:divide-slate-700">
                {filteredCards.length > 0 ? filteredCards.map((card, index) => (
                  <tr key={card.id} className={`transition-colors hover:bg-laundry-blue-100/60 dark:hover:bg-slate-700/60`}>
                    ...
                    <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-700 dark:text-slate-300">{new Date(card.createdAt).toLocaleDateString('pt-BR')}</td>
